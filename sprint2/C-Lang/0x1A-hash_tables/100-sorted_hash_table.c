...
{
	shash_node_t *tmp;
	/* if empty hash table, initialise head and tail nodes */
	if (!(ht->shead))
	{
		ht->shead = node;
		ht->stail = node;
...
/**
 * create_and_add_node - malloc, set values, insert node into hash talble
 * @ht: sorted hash table
 * @key: key, cant be empty string
 * @value: value
 * @idx: index to inset into
 * Return: 1 if success, 0 if fail
 */

int create_and_add_node(shash_table_t *ht, const char *key, const char *value, unsigned long int idx)
{
...
char *shash_table_get(const shash_table_t *ht, const char *key)
{
	...
	/* find index in table where key is */
	/* look through list to find matching key for value */
...
/**
 * shash_table_print_rev - print key/values of sorted hash table in reverse order
 * @ht: hash table
 */
